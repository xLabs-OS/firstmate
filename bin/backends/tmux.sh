...
#
# Worktree acquisition (fm-spawn.sh leasing the worktree from
# `treehouse get --lease`, cd-ing the pane into it, and confirming arrival
# through the current-path probe) stays inline in fm-spawn.sh: P1 scopes only
# the session provider, not the worktree provider, so fm-spawn.sh drives that
# part with these same send/current-path primitives.
#
...
#     the pane cds into the worktree, which would break name-based targeting.
# The returned window id lets callers target the window even if its name is ever
# lost, so worktree confirmation cannot fall back to the active client's window.
...
# empty on any tmux error. Mirrors fm-spawn.sh's worktree-confirmation poll:

...
test_local_only_follows_selected_path_without_stacked_review() {
  local home id brief
  home="$TMP_ROOT/local-only-authority-home"
  write_registry "$home"
  id="brief-local-authority-a4"
  FM_HOME="$home" "$ROOT/bin/fm-brief.sh" "$id" local-proj >/dev/null 2>&1
  brief="$home/data/$id/brief.md"
  assert_grep "The captain approves the ready branch, then firstmate merges it into local \`main\` through the guarded fast-forward path." "$brief" \
    "local-only brief lost captain approval and guarded landing"
  assert_no_grep "Firstmate then reviews your branch diff" "$brief" \
    "local-only brief retained a personal review stacked on the selected delivery path"
  pass "fm-brief.sh: local-only uses captain approval and guarded landing without stacked review"
}

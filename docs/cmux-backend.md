...
`current_directory` follows a top-level shell `cd` but never a `cd` made inside a foreground subshell.
Spawn-time worktree confirmation sends begin and end markers around `pwd`, captures the marked block, and joins wrapped path lines.

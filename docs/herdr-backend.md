...
The repositioning move-to-last preserves every surviving workspace's relative order, and removal is confirmed against the exact moved workspace rather than inferred from pane disappearance: Herdr removes the emptied workspace on a tick after reaping its dead pane and tab, so the confirmation polls within the pane-death budget instead of trusting one immediate read.
A close that fails with the pane still alive makes one verified attempt under the same session lock to roll the not-actually-doomed workspace back to its exact original position, and warns loudly with retained records if that rollback cannot restore the verified original order.
Once the pane's death is confirmed the doomed workspace is never rolled back: it stays parked behind the focused one, reported loudly, because restoring it ahead of the focused workspace would re-arm the stale-index focus jump at the moment the lagging removal completes.
...
The matching journal is retired only after the exact pane is positively confirmed gone; a close left unconfirmed because the pane itself could not be proven gone retains the journal, while a pane proven gone retires it even when focus restoration reported an error or the emptied workspace's own removal was still unconfirmed after the close.
